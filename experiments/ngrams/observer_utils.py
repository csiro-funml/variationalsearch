...
def plot_sum_y(obs: SimpleObserverForMads, ax: plt.Axes, start_from: int = 0):
    sumy = np.array(obs.y_s).squeeze().sum(axis=1)
    asumy = np.maximum.accumulate(sumy)
    ax.plot(asumy[start_from:])
    ax.set_xlabel("Number of evaluations")
    ax.set_ylabel("Best sum found")
