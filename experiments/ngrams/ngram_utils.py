...
RESIDUE_ALPHABET = (
    ["[PAD]", "[CLS]", "[UNK]", "[MASK]", "[SEP]"] + AMINO_ACIDS + ["0"]
)
...
APTAMER_ALPHABET = (
    ["[PAD]", "[CLS]", "[UNK]", "[MASK]", "[SEP]"] + APTAMER_BASES + ["0"]
)

TOY_VOCAB = ["A", "R"]
TOY_ALPHABET = (
    ["[PAD]", "[CLS]", "[UNK]", "[MASK]", "[SEP]"] + TOY_VOCAB + ["0"]
)
...
class IntTokenizer:
    def __init__(
        ...
        self.non_special_idxs = [
            self.convert_token_to_id(t) for t in non_special_vocab
        ]
        self.special_idxs = [
            self.convert_token_to_id(t) for t in self.special_vocab
        ]
    ...
    def set_sampling_vocab(self, sampling_vocab=None, max_ngram_size=1):
        if sampling_vocab is None:
            ...
            self.inverse_lookup = {
                i: a for (i, a) in enumerate(self.full_vocab)
            }
...
def random_strings(
    ...
    strs = []
    for _ in range(num):
        length = rand.randint(min_len, max_len + 1)
        idx = rand.choice(len(alphabet), size=length, replace=True)
        strs.append("".join([alphabet[i] for i in idx]))
    return np.array(strs)


def str_to_tokens(str_array, tokenizer, use_sep=True):
    tokens = [torch.tensor(tokenizer.encode(x, use_sep)) for x in str_array]
...
def tokens_to_str(tok_idx_array, tokenizer):
    str_array = np.array(
        [
            tokenizer.decode(token_ids).replace(" ", "")
            for token_ids in tok_idx_array
        ]
    )
    return str_array
